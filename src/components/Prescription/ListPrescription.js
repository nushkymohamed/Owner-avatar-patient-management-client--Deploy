...
function ListPrescriptions() {
  ...
  useEffect(() => {
    axios
      .get(
        'https://johnsmedicals-patient-management-service.onrender.com/prescriptions'
      )
      .then((response) => {
      ...
      renderCell: (params) => {
        ...
        for (let index = 0; index < params.row.files.length; index++) {
          filesWithCommas.push(
            <a
              key={index}
              href={`https://johnsmedicals-patient-management-service.onrender.com/${params.row.files[index]}`}
              target="_blank"
              rel="noreferrer"
            >
              File {index + 1}
